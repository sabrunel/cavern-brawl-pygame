...
class Stage():
    def __init__(self):
        ...
        # Stage states
        self.user_clicked = False # User input
        self.is_attacking = False
        self.potion_clicked = False
        self.target = None


    def create_fighters(self):
        #Create an instance of each fighter type
        self.hero = Fighter(200, 350, 'Hero', 10, 30, 3)
        self.bat = Fighter(450, 320, 'Bat', 6, 10, 3)
        self.plaguedoctor = Fighter(600, 350,'PlagueDoctor', 8, 20, 3)
    ...
    def draw_names(self):
        # Draw hero name
        draw_text(self.display_surface,f'{self.hero.name}', self.font, TEXT_COLOR, 10, 10)

        # Draw enemy names
        for count, enemy in enumerate(self.enemy_list):
            draw_text(self.display_surface,f'{enemy.name}', self.font, TEXT_COLOR, 640, 10 + count * 50)

    def create_healthbars(self):
        self.hero_healthbar = HealthBar( 10, 35, self.hero.hp, self.hero.max_hp)
        self.bat_healthbar = HealthBar(640, 35, self.bat.hp, self.bat.max_hp)
        self.plaguedoctor_healthbar = HealthBar(640, 85, self.plaguedoctor.hp, self.plaguedoctor.max_hp)  

    def player_input(self, bool):
        # Listen for mouse click
        self.user_clicked = bool

    def set_potion(self):
        if self.potion_btn.draw():
            self.potion_clicked = True
    ...
    def set_target(self):
        ...
        for index, enemy in enumerate(self.enemy_list):
            if enemy.rect.collidepoint(pos):
                ...
                if self.user_clicked and enemy.alive:
            ...
            if self.current_turn == 1:
                ...
                if self.action_cd >= self.action_wait_time:
                    ...
                    if self.is_attacking == True and self.target != None:
                        ...
                        dmg_txt = CombatText(self.target.rect.centerx, self.target.rect.y, self.font,  str(self.hero.damage), HEALTH_RED)
                        self.combat_text_group.add(dmg_txt)  

                    # Use potion
                    if self.potion_clicked == True:
                        if self.hero.potions > 0:
                            ...
                            heal_txt = CombatText(self.hero.rect.centerx, self.hero.rect.y, self.font,  str(heal_amount), HEALTH_GREEN)
                ...
                if enemy.alive:
                    ...
                    if self.action_cd >= self.action_wait_time:
                        ...
                        if (enemy.hp / enemy.max_hp) < 0.5 and enemy.potions > 0:
                            ...
                            # Create the healing text
                            heal_txt = CombatText(enemy.rect.centerx, enemy.rect.y,  self.font, str(heal_amount), HEALTH_GREEN)
                            self.combat_text_group.add(heal_txt)
                        ...
                        # Bandit attack
                            ...
                            # Create the damage text
                            dmg_txt = CombatText(self.hero.rect.centerx, self.hero.rect.y, self.font, str(enemy.damage), HEALTH_RED)
                            self.combat_text_group.add(dmg_txt)                 
    ...
    def reset_turn(self):   
        # Reset once all fighter had a turn
    ...
    def draw(self):
        ...
        draw_text(self.display_surface, str(self.hero.potions), self.font, TEXT_COLOR, 55, 400)
